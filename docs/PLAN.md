...
- EntitySpec Pydantic model (the spec describing specs).
- YAML loader + schema validation.
- `spec_hash` calculator.
...
- Test infrastructure:
  ...
  - Cassette recorder/player for paid backends (VCR-style).
  - Ollama sidecar integration test running on every PR.
...
- `ExtractionFailed` exception hierarchy.
