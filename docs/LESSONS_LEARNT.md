...
## LL-009 — Local-only safety gate blocked its own CI enforcement

- **Date:** 2026-06-09
- **Tags:** `ci`
- **Symptom:** `scripts/check_branch_guard.py` (the pre-commit gate that refuses commits on `main`/`master` per RULES.md §1.0) runs unconditionally. CI workflows that legitimately operate on `main` — checking out the default branch, replaying a tagged release commit, running gates against the merge result — tripped the same guard inside the runner, failing the job for a "commit on main" that was never an interactive human commit at all. The gate designed to protect the branch locally became a false-positive blocker for the automation that runs *after* the branch is legitimately advanced.
- **Root cause:** The guard conflated two distinct execution contexts that happen to share a command. Locally, "a commit is being made on `main`" means a human bypassed the PR flow and should be stopped. In CI, the same code path runs as a side effect of checkout/replay on the default branch and carries no such intent. A safety gate whose whole purpose is enforcing the *local* PR-only workflow had no notion of "am I running in CI," so it applied a local-workflow rule to an environment where that rule is meaningless. This is the inverse failure of LL-005: there the escape hatch was *too easy* to trip (any truthy string); here the gate had *no* legitimate exemption for the one context that needs it.
- **Fix:** Made `check_branch_guard.py` no-op when `CI=true` or `GITHUB_ACTIONS=true` (the two standard CI sentinels), so the guard enforces the PR-only workflow on developer machines while staying out of the way of the CI that runs against `main` by design. Shipped in PR #17 (`a85df0a`→`5aa0f55`); the exemption was then written into the RULES.md §1.0 contract via PR #18 so the carve-out is specified, not just coded. Distinct from LL-005's `ALLOW_MAIN_COMMIT` string-equality fix — that hardened the *manual* opt-in; this adds an *automatic* environment-scoped exemption.
- **Prevention:** A gate that enforces a workflow rule must encode *which execution context the rule applies to*, not just the condition it checks. Before shipping a guard that runs on every commit, ask "does this also run in CI, and is the rule still correct there?" — local-workflow gates almost always need a `CI`/`GITHUB_ACTIONS` carve-out because the automation legitimately does things the local rule forbids. Always pair the code carve-out with a spec line (RULES.md) so the exemption is contract, not folklore: an undocumented `if CI: return` reads as a bug to the next reviewer and invites "tighten" reverts that re-break CI.

## LL-010 — Unvalidated external numeric field (OAuth `expires_in`) breaks the refresh loop

- **Date:** 2026-06-11
- **Tags:** `runtime`
- **Symptom:** `OAuthProvider._refresh` computed `expires_in = float(payload.get("expires_in", 3600))` directly from the OAuth token endpoint's JSON response. A hostile or buggy server returning a negative value (e.g. `-1`) puts `expires_at` in the past, so the token is treated as expired on every `resolve` → tight refresh loop hammering the token endpoint. A non-finite value (`Infinity`) makes the token never refresh. A non-numeric value (`"not-a-number"`) raises an uncaught `ValueError` that escapes the `CredentialError` boundary as a raw stack trace at adapter-construction time.
- **Root cause:** An external API response field was coerced with bare `float()` and fed into arithmetic (`now + expires_in`) without a parse/finiteness/sign guard. The provider validated the *presence* of `access_token` but treated the adjacent numeric `expires_in` as trusted. An external response field is system-boundary input — the same trust class as user input — yet the numeric one skipped validation.
- **Fix:** Wrapped the coercion in `try/except (TypeError, ValueError)` raising a single-line `CredentialError`, and added `if not math.isfinite(expires_in) or expires_in <= 0: raise CredentialError(...)`. Both raise *before* `store[backend] = updated` / `_write_token_store`, so the bad path fails closed and the cached token is preserved. Added `tests/test_credentials.py::test_oauth_refresh_invalid_expires_in_raises` (parametrized: negative / zero / non-finite / non-numeric).
- **Prevention:** Treat every numeric field read from an external API response as untrusted boundary input — parse defensively (`try/except` around the coercion) and range-check (`math.isfinite` + sign) before it drives arithmetic that controls flow. A value that sets an expiry, retry delay, or loop bound must be validated: an out-of-range value silently degrades into a denial-of-service (tight loop) or a stuck state (never refresh). Mirror the existing presence-checks on tokens with value-checks on the numbers next to them.

## LL-011 — Error position resolved against a different structure than the one validated

- **Date:** 2026-06-12
- **Tags:** `spec`
- **Symptom:** When a child spec `extends` a parent and a *parent-only* (inherited) field failed validation (e.g. a parent field with `type: badtype`), `load_spec` reported the error at the **child** file, line 1 — never the parent file or the offending parent line. The `file:line:col` diagnostic sent the user to edit an unrelated location: in the worst case it pointed at a real-but-wrong child field that merely happened to share the offending field's list index.
- **Root cause:** `load_spec` validated the *flattened* spec (parent + child merged by field `name`) but resolved the Pydantic error `loc` path (e.g. `("fields", 0, "type")`) against the *child's* raw mark-annotated mapping via `_lookup_loc(raw=child, loc)`. `_flatten` stripped the `__line__`/`__column__` marks up front, so only the child's pre-flatten raw structure carried any positions to look up. But `_merge_named_list` interleaves parent and child fields by name, so index `0` in the flattened `fields` list is not index `0` in the child's raw `fields` list. The lookup either walked off the end of the child list (→ fell back to the child root, line 1) or resolved to a *different, unrelated* child field at that index — wrong file **and** wrong node. An error position computed against structure A cannot be resolved against structure B when A and B have different shapes, and flatten + merge-by-name guarantees the shapes differ whenever inheritance reorders, replaces, or contributes fields.
- **Fix:** Threaded origin metadata through the flatten so the error `loc` resolves against a structure whose shape *exactly matches* the validated flattened spec. `_tag_file` stamps every mapping with a transient `__file__` mark in `_load_raw`; `_flatten` now keeps the `__line__`/`__column__`/`__file__` marks through `_merge` instead of stripping them at entry; `load_spec` strips marks only into a throwaway `flattened` copy for `model_validate`, then resolves errors with `_lookup_origin(marked, loc)` against the still-marked flattened structure and reports `loc_file or p`. Each node now carries its true origin file/line, so an inherited-field error points at the parent. Added `tests/test_spec_loader.py::test_load_spec_extends_inherited_field_error_points_to_parent`. Marks are stripped before validation, so `EntitySpec` (extra="forbid") never sees them and Q3 exit criterion 6 ("field provenance NOT populated on the model") stays satisfied — the file tags are loader-internal and transient.
- **Prevention:** When you compute an error position (index path, offset, line map) against one representation and then look it up in another, the two representations must be structurally identical, or the lookup silently resolves to the wrong node. If a transform (flatten, merge, sort, dedupe) sits between "the structure that was validated" and "the structure you index for diagnostics," resolve the diagnostic against the *post-transform* structure and carry per-node origin metadata through the transform — do not index the pre-transform source. A position lookup that returns *something* for an out-of-range or reordered index is more dangerous than one that fails loudly: it produces a confidently-wrong `file:line` that sends the user to edit unrelated code.

## LL-012 — Runtime data resolved by `__file__` parent-walking breaks in non-editable/wheel installs

- **Date:** 2026-06-14
- **Tags:** `container`, `release`
- **Symptom:** The MCP server resolved its example specs with `Path(__file__).resolve().parents[3] / "examples"`, which points at the repo root in an editable/dev checkout. Under a real wheel install (`pip install prompiler`, or the container's `uv sync --frozen --no-dev --no-editable`, which materializes the project as a wheel into `.venv`), the package lives in `site-packages`, so `parents[3]` resolved to `.../lib/python3.11` and `.../examples` did not exist. `load_examples()` then called `Path(examples_dir).iterdir()` → `FileNotFoundError` and the server died on startup. The bug was invisible to the entire test suite and to local `uv run` because both run against the editable tree where the parent-walk happens to land on the repo root.
- **Root cause:** Two coupled mistakes. (1) Runtime data was located by walking up from `__file__`, which encodes the *source-tree* layout, not the *installed* layout — the two differ for any non-editable install. (2) The data was never declared as package data, so even a correct resolver would have found nothing: the wheel shipped no specs. The dev workflow (editable install + repo-root `examples/`) masked both, so every gate stayed green while the shippable artifact was broken.
- **Fix:** Ship the specs as package data and resolve them from the installed package. `_resolve_examples_dir()` now prefers `importlib.resources.files("prompiler") / "examples"` when it `is_dir()`, falling back to the `parents[3]` repo-root path only for editable/dev. The 9 `examples/*.yaml` are force-included into the wheel via `[tool.hatch.build.targets.wheel.force-include]` (keys cannot glob, so each is listed explicitly), and the builder stage gained `COPY examples ./examples` so the force-include source files are present at build time. A meta-test (`test_every_example_spec_is_force_included_in_wheel`) fails if a new `examples/*.yaml` is added without a matching force-include entry, and two resolver tests cover the packaged-preferred and editable-fallback branches. Verified by building the wheel (specs present under `prompiler/examples/`) and running the container: in-container `/healthz` returns `200` and all 9 specs resolve via `importlib.resources`.
- **Prevention:** Never locate runtime data by parent-walking from `__file__` — that path assumes the source-tree layout and silently diverges from the installed layout for any wheel/non-editable install. Declare runtime data as package data and resolve it with `importlib.resources.files(<pkg>)`. Treat the editable dev install as a *masking* environment: a green test suite there proves nothing about the shipped wheel. For any "where does my data live at runtime" change, add a check that exercises the *installed* artifact (build the wheel and inspect it, or run the container), and a meta-test that fails when newly added data files are not declared for packaging.

## LL-013 — Two hand-bumped copies of the release version drift without a guard

- **Date:** 2026-06-15
- **Tags:** `release`
- **Symptom:** The package version lived as a literal in BOTH `pyproject.toml` L3 (`version = "0.2.0"`) and `src/prompiler/__init__.py` L5 (`__version__ = "0.2.0"`), kept in sync only by the Q5 K1 "bump both in lockstep" instruction. Nothing had failed yet, but the two could silently diverge: a release that bumps `pyproject.toml` and forgets `__init__.py` (or vice-versa) ships a wheel whose stamped metadata disagrees with its runtime `__version__`, and `release.yml`'s verify-tag step (`tag == "v$__version__"`) would then assert the tag against a stale literal.
- **Root cause:** A single fact (the release version) was stored in two places with manual lockstep as the only consistency mechanism. Hand-maintained duplication of a value is a drift hazard regardless of whether the drift has occurred yet — the contract "remember to edit both" is unenforceable and degrades to "edit one." The build backend (hatchling) already reads `pyproject.toml` as the authoritative source when stamping the wheel, so the `__init__.py` literal was a second, redundant copy with no authority.
- **Fix:** Made `pyproject.toml` the single source. `__version__` now derives from the installed package metadata via `importlib.metadata.version("prompiler")`, with a `PackageNotFoundError` → `"0.0.0+unknown"` sentinel for an un-installed source checkout. Removed the `__init__.py` literal entirely. Added `tests/test_smoke.py::test_version_derived_from_metadata` (line-anchored source-grep asserting no module-level version literal remains) to block reintroduction.
- **Prevention:** Store any release-identifying value in exactly one place and derive every other use from it. For a Python package that place is `pyproject.toml [project].version`; expose it at runtime with `importlib.metadata.version(<pkg>)`, never a second literal. Because that derivation reads *installed* metadata (which an editable checkout can lag behind a fresh `pyproject` bump), pair it with a drift guard that turns the lag into a loud test failure rather than silent disagreement. Treat "remember to bump both" instructions as a smell: they are unenforced lockstep and will eventually be half-applied.
